...
# Вся рабочая информация приложения хранится в папке 'app_data'
# ============================================
from pathlib import Path

# Определяем базовую директорию - папку, где находится этот файл
_base_dir: Path = Path(__file__).resolve().parent

# Папка для данных приложения
_app_data_dir: Path = _base_dir / "app_data"
# Подпапка для файлов сессий Telethon
_sessions_dir: Path = _app_data_dir / "sessions"
# Подпапка для логов
_logs_dir: Path = _app_data_dir / "logs"

# Создаем необходимые каталоги, если они еще не существуют
for _d in (_app_data_dir, _sessions_dir, _logs_dir):
    _d.mkdir(parents=True, exist_ok=True)

# Пути к конфигурационным файлам
# Файл с настройками API и задержки
_config_file: Path = _app_data_dir / "config.json"
# Файл с основными данными (списки групп, тем, шаблонов и т.д.)
_app_data_file: Path = _app_data_dir / "app_data.json"
    ...
    def start(self, app):
        ...
        def runner():
            ...
            try:
                ...
                try:
                    # Завершаем все задачи, используя современный вызов asyncio.all_tasks() без указания цикла
...
# Используем папку для логов внутри директории данных приложения
...
# КОНФИГУРАЦИЯ
# ============================================
# Пути к файлам конфигурации и данным. Используем директорию данных приложения
USER_CONFIG = str(_config_file)
APP_DATA_FILE = str(_app_data_file)
...
async def init_client(app, api_id, api_hash, phone):
    # Имя сессии очищаем от символов '+' и пробелов, затем сохраняем в подпапке sessions
    session_name = f"session_{phone.strip().replace('+', '').replace(' ', '')}"
    # Путь к файлу сессии (без расширения .session) внутри папки sessions
    session_path = os.path.join(str(_sessions_dir), session_name)
...
class TelegramSenderApp:
    ...
    def create_groups_manager(self, parent):
        ...
        # Поля для ввода групп: используем те же наименования, что и в форме тем (ID группы, Название, Номер клиента)
        fields = [("ID группы:", "group_id_entry"), ("Название:", "group_name_entry"),
