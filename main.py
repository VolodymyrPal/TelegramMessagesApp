...
class TelegramSenderApp:
    def __init__(self, root):
        ...
        # Lock to serialize access to Telethon session (sqlite)
    ...
    def _mk_button_styles(self):
        ...
        variants = {'Primary': self.colors['primary'], 'Success': self.colors['success'],
                    'Danger': self.colors['danger'], 'Secondary': '#64748b'}
        for name, color in variants.items():
            darker, pressed = self._adjust_color(color, 0.9), self._adjust_color(color, 0.8)
            stylename = f'Btn.{name}.TButton'
            style.configure(stylename, background=color, foreground='#ffffff', **base)
            style.map(stylename, background=[('active', darker), ('pressed', pressed), ('disabled', '#cbd5e1')],
                      foreground=[('disabled', '#ffffff')])

    def _adjust_color(self, color, factor=0.9):
        try:
            r16, g16, b16 = self.root.winfo_rgb(color)
            r, g, b = (max(0, min(255, int((c / 65535) * 255 * factor))) for c in (r16, g16, b16))
            return f'#{r:02x}{g:02x}{b:02x}'
        except Exception:
            return color

    def create_button(self, parent, text, command, variant='primary', **kwargs):
        style_map = {'primary': 'Btn.Primary.TButton', 'success': 'Btn.Success.TButton',
        ...
        if not style_name:
            ...
            s.configure(style_name, background=color, foreground='#ffffff', font=('Segoe UI', 10, 'bold'),
                        padding=(16, 10))
            s.map(style_name, background=[('active', darker), ('pressed', pressed), ('disabled', '#cbd5e1')],
                  foreground=[('disabled', '#ffffff')])
    ...
    def _bind_clipboard_shortcuts(self, widget):
        bindings = [('<Control-c>', '<<Copy>>'), ('<Control-C>', '<<Copy>>'), ('<Control-Insert>', '<<Copy>>'),
                    ('<Control-v>', '<<Paste>>'), ('<Control-V>', '<<Paste>>'), ('<Shift-Insert>', '<<Paste>>'),
                    ('<Control-x>', '<<Cut>>'), ('<Control-X>', '<<Cut>>'), ('<Shift-Delete>', '<<Cut>>'),
                    ('<Command-c>', '<<Copy>>'), ('<Command-C>', '<<Copy>>'), ('<Command-v>', '<<Paste>>'),
                    ('<Command-V>', '<<Paste>>'), ('<Command-x>', '<<Cut>>'), ('<Command-X>', '<<Cut>>')]
        for seq, ev in bindings:
            widget.bind(seq, lambda e, ev=ev: (widget.event_generate(ev), 'break'))
        return widget
    ...
    def mk_text(self, parent, **kwargs):
        txt = scrolledtext.ScrolledText(parent, wrap=tk.WORD, font=('Segoe UI', 10),
                                        bg=self.colors['input_bg'], fg=self.colors['input_fg'], relief='solid',
                                        bd=1, insertbackground=self.colors['text'], **kwargs)
    ...
    def mk_listbox(self, parent, height=12):
        ...
        listbox = tk.Listbox(frame, font=('Segoe UI', 9), bg=self.colors['input_bg'], fg=self.colors['input_fg'],
                             ...
                             selectforeground='#ffffff', height=height, exportselection=False)
    ...
    def create_widgets(self):
        ...
        tabs = {"  ⚙️  Настройки  ": self.create_settings_tab,
    ...
    def create_settings_tab(self, parent):
        container = tk.Frame(parent, bg=self.colors['bg']); container.place(relx=0.5, rely=0.5, anchor='center')
        card = self.create_card(container, "🔐  Настройки Telegram API"); card.pack(padx=40, pady=20)
        fields_frame = tk.Frame(card, bg=self.colors['card']); fields_frame.pack(fill="x", pady=10)
        def field(label, row):
            self.mk_label(fields_frame, label, bold=True).grid(row=row, column=0, sticky='w', pady=12, padx=(0, 15))
            e = self.mk_entry(fields_frame); e.grid(row=row, column=1, sticky='ew', pady=12); e.config(width=40)
            return e
        self.api_id_entry, self.api_hash_entry = field("API ID:", 0), field("API Hash:", 1)
        self.phone_entry, self.rate_delay_entry = field("Телефон:", 2), field("Задержка (сек на отправку):", 3)
        fields_frame.columnconfigure(1, weight=1)
        btn_frame = tk.Frame(card, bg=self.colors['card']); btn_frame.pack(pady=20)
        ...
        self.settings_status = tk.Label(card, text="", bg=self.colors['card'], fg=self.colors['text'],
                                        font=('Segoe UI', 10)); self.settings_status.pack(pady=10)

    def create_manage_tab(self, parent):
        main = tk.Frame(parent, bg=self.colors['bg']); main.pack(fill="both", expand=True, padx=20, pady=20)
        main.grid_columnconfigure((0, 1, 2), weight=1); main.grid_rowconfigure(0, weight=1)
    ...
    def create_tags_manager(self, parent):
        ...
        form = tk.Frame(card, bg=self.colors['card']); form.pack(fill='x', pady=5)
        self.mk_label(form, "Новый тег:", bold=True).pack(side='left')
        self.tag_name_entry = self.mk_entry(form); self.tag_name_entry.pack(side='left', fill='x', expand=True, padx=(10, 0))
        btns = tk.Frame(card, bg=self.colors['card']); btns.pack(fill='x', pady=10)
        self.create_button(btns, "➕", self.add_tag, variant='success', width=5).pack(side='left', expand=True, fill='x', padx=2)
        self.create_button(btns, "❌", self.delete_tag, variant='danger', width=5).pack(side='left', expand=True, fill='x', padx=2)
        return card
    ...
    def create_groups_manager(self, parent):
        ...
        form = tk.Frame(card, bg=self.colors['card']); form.pack(fill='x', pady=10); form.columnconfigure(1, weight=1)
        for i, (text, name) in enumerate([("ID:", "group_id_entry"), ("Название:", "group_name_entry"), ("Кабинет:", "group_cabinet_entry")]):
            self.mk_label(form, text, bold=True).grid(row=i, column=0, sticky='w', pady=4)
            setattr(self, name, self.mk_entry(form)); getattr(self, name).grid(row=i, column=1, sticky='ew', pady=4, padx=(10, 0))
        btns = tk.Frame(card, bg=self.colors['card']); btns.pack(fill='x', pady=10)
        self.create_button(btns, "➕", self.add_group, variant='success', width=5).pack(side='left', expand=True, fill='x', padx=2)
        self.create_button(btns, "✏️", lambda: self.edit_item_tags('group'), variant='primary', width=5).pack(side='left', expand=True, fill='x', padx=2)
        self.create_button(btns, "❌", self.delete_group, variant='danger', width=5).pack(side='left', expand=True, fill='x', padx=2)
        return card
    ...
    def create_themes_manager(self, parent):
        ...
        form = tk.Frame(card, bg=self.colors['card']); form.pack(fill='x', pady=10); form.columnconfigure(1, weight=1)
        for i, (text, name) in enumerate([("ID группы:", "theme_group_id_entry"), ("ID темы:", "theme_topic_id_entry"), ("Название:", "theme_name_entry"), ("Кабинет:", "theme_cabinet_entry")]):
            self.mk_label(form, text, bold=True).grid(row=i, column=0, sticky='w', pady=4)
            setattr(self, name, self.mk_entry(form)); getattr(self, name).grid(row=i, column=1, sticky='ew', pady=4, padx=(10, 0))
        btns = tk.Frame(card, bg=self.colors['card']); btns.pack(fill='x', pady=10)
        self.create_button(btns, "➕", self.add_theme, variant='success', width=5).pack(side='left', expand=True, fill='x', padx=2)
        self.create_button(btns, "✏️", lambda: self.edit_item_tags('theme'), variant='primary', width=5).pack(side='left', expand=True, fill='x', padx=2)
        self.create_button(btns, "❌", self.delete_theme, variant='danger', width=5).pack(side='left', expand=True, fill='x', padx=2)
        return card

    def create_fetch_tab(self, parent):
        container = tk.Frame(parent, bg=self.colors['bg']); container.place(relx=0.5, rely=0.5, anchor='center')
        card = self.create_card(container, "📥  Получить список всех групп"); card.pack(padx=40, pady=20)
        ...
        self.fetched_groups_listbox = self.mk_listbox(card); self.fetched_groups_listbox.config(selectmode='multiple', width=70)
        self.create_button(card, "➕  Добавить выбранные в список", self.add_fetched_groups, variant='success').pack(pady=15)

    def create_topic_checker_tab(self, parent):
        container = tk.Frame(parent, bg=self.colors['bg']); container.place(relx=0.5, rely=0.5, anchor='center')
        card = self.create_card(container, "🔎  Получить темы из группы-форума"); card.pack(padx=40, pady=20, fill='x')
        ...
        self.fetched_topics_listbox = self.mk_listbox(card); self.fetched_topics_listbox.config(selectmode='multiple', width=70)
        self.create_button(card, "➕  Добавить выбранные темы в список", self.add_fetched_topics, variant='success').pack(pady=15)

    def create_sending_tab(self, parent):
        main = tk.Frame(parent, bg=self.colors['bg']); main.pack(fill='both', expand=True, padx=20, pady=20)
        self.left_col_sending = tk.Frame(main, bg=self.colors['bg']); self.left_col_sending.pack(side='left', fill='both', expand=True, padx=(0, 10))
        right = tk.Frame(main, bg=self.colors['bg']); right.pack(side='right', fill='both', expand=True, padx=(10, 0))
        templates_card = self.create_card(right, "📝  Шаблоны сообщений"); templates_card.pack(fill='x', pady=(0, 10))
        self.create_templates_manager(templates_card)
        msg_card = self.create_card(right, "✉️  Текст сообщения"); msg_card.pack(fill='both', expand=True, pady=(0, 10))
        self.message_text = self.mk_text(msg_card, height=10); self.message_text.pack(fill='both', expand=True)
        self.send_btn = self.create_button(right, "📨  Отправить сообщения", self.prepare_send, variant='success')
        self.send_btn.config(width=20); self.send_btn.pack(pady=10, fill='x')
        log_card = self.create_card(right, "📊  Лог отправки"); log_card.pack(fill='both', expand=True)
        self.log_text = scrolledtext.ScrolledText(log_card, height=8, state='disabled', font=('Consolas', 9),
                                                  ...
                                                  relief='solid', bd=1); self.log_text.pack(fill='both', expand=True)

    def create_templates_manager(self, parent):
        self.templates_listbox = self.mk_listbox(parent, height=5)
        btns = tk.Frame(parent, bg=self.colors['card']); btns.pack(fill='x', pady=10)
        self.create_button(btns, "📥", self.use_template, variant='primary', width=5).pack(side='left', expand=True, fill='x', padx=2)
        self.create_button(btns, "💾", self.save_template, variant='success', width=5).pack(side='left', expand=True, fill='x', padx=2)
        self.create_button(btns, "❌", self.delete_template, variant='danger', width=5).pack(side='left', expand=True, fill='x', padx=2)

    def build_sending_lists(self, parent):
        for w in parent.winfo_children(): w.destroy()
        tags_card = self.create_card(parent, "🏷️  Фильтр по тегам"); tags_card.pack(fill='x', pady=(0, 10))
        tags_outer = tk.Frame(tags_card, bg=self.colors['tag_filter_bg'], bd=1, relief='solid', borderwidth=0); tags_outer.pack(fill='x', pady=5, padx=0)
        tags_frame = tk.Frame(tags_outer, bg=self.colors['tag_filter_bg']); tags_frame.pack(fill='x', padx=5, pady=5)
        ...
        def update_all_tags_state():
            self.all_tags_var.set(all(var.get() for var, _ in self.tag_filter_vars) if self.tag_filter_vars else True)
            self.filter_sending_lists()
        all_cb = tk.Checkbutton(tags_frame, text="Все", variable=self.all_tags_var, command=toggle_all_tags,
                                ...
                                activeforeground=self.colors['text']); all_cb.pack(side='left', padx=5)
        for tag in self.app_data["tags"]:
            var = tk.BooleanVar(value=True); self.tag_filter_vars.append((var, tag))
            cb = self.mk_checkbutton(tags_frame, tag, var, bg_card=False); cb.config(command=update_all_tags_state)
            cb.pack(side='left', padx=5)
        self.scrollable_groups_frame_container = self.create_card(parent, "📁  Выбор групп"); self.scrollable_groups_frame_container.pack(fill='both', expand=True, pady=(0, 10))
        self.scrollable_themes_frame_container = self.create_card(parent, "🧵  Выбор тем"); self.scrollable_themes_frame_container.pack(fill='both', expand=True)
        btn_select = tk.Frame(parent, bg=self.colors['bg']); btn_select.pack(fill='x', pady=10)
        self.create_button(btn_select, "✓", self.select_all, variant='primary', width=10).pack(side='left', padx=5, expand=True, fill='x')
        self.create_button(btn_select, "✗", self.deselect_all, variant='secondary', width=10).pack(side='left', padx=5, expand=True, fill='x')
        self.filter_sending_lists()
    ...
    def filter_sending_lists(self):
        ...
        def populate(container, items, is_group):
            ...
            if is_group: self.group_vars = item_vars
            else: self.theme_vars = item_vars
    ...
    def refresh_all_lists(self):
        listbox_data = {self.tags_listbox: self.app_data["tags"],
                        self.groups_listbox: [f"{g['name']} | ID: {g['id']}" for g in self.app_data["groups"]],
                        self.themes_listbox: [f"{t['name']} | Группа: {t['group_id']}" for t in self.app_data["themes"]],
                        self.templates_listbox: [tpl["name"] for tpl in self.app_data["templates"]]}
        for listbox, data in listbox_data.items():
    ...
    def add_tag(self):
        ...
        self.app_data["tags"].append(tag_name); save_app_data(self.app_data); self.refresh_all_lists()
        self.tag_name_entry.delete(0, tk.END); messagebox.showinfo("Успех", "Тег добавлен!")
    ...
    def delete_tag(self):
        ...
        if messagebox.askyesno("Подтверждение", f"Удалить тег '{tag_to_delete}'? Он также будет удален из всех групп и тем."):
            ...
            save_app_data(self.app_data); self.refresh_all_lists(); messagebox.showinfo("Успех", "Тег удален!")

    def add_group(self):
        try:
            gid = int(self.group_id_entry.get().strip())
            name, cab = self.group_name_entry.get().strip(), self.group_cabinet_entry.get().strip()
        ...
        except ValueError: messagebox.showerror("Ошибка", "ID группы должен быть числом!")
    ...
    def delete_group(self):
        ...
        if messagebox.askyesno("Подтверждение", "Удалить выбранную группу?"):
            del self.app_data["groups"][sel[0]]; save_app_data(self.app_data); self.refresh_all_lists(); messagebox.showinfo("Успех", "Группа удалена!")

    def add_theme(self):
        try:
            gid, tid = int(self.theme_group_id_entry.get().strip()), int(self.theme_topic_id_entry.get().strip())
            name, cab = self.theme_name_entry.get().strip(), self.theme_cabinet_entry.get().strip()
            ...
            save_app_data(self.app_data); self.refresh_all_lists()
            for e in [self.theme_group_id_entry, self.theme_topic_id_entry, self.theme_name_entry, self.theme_cabinet_entry]: e.delete(0, tk.END)
            messagebox.showinfo("Успех", "Тема добавлена!")
        except ValueError: messagebox.showerror("Ошибка", "ID группы и темы должны быть числами!")
    ...
    def delete_theme(self):
        ...
        if messagebox.askyesno("Подтверждение", "Удалить выбранную тему?"):
            del self.app_data["themes"][sel[0]]; save_app_data(self.app_data); self.refresh_all_lists(); messagebox.showinfo("Успех", "Тема удалена!")
    ...
    def edit_item_tags(self, item_type):
        ...
        if not sel: return messagebox.showwarning("Внимание", f"Выберите {'группу' if item_type == 'group' else 'тему'} для редактирования!")
        item = data_list[sel[0]]
        dialog = tk.Toplevel(self.root); dialog.title("Редактировать теги"); dialog.configure(bg=self.colors['bg']); dialog.transient(self.root); dialog.grab_set()
        self.mk_label(dialog, f"Теги для '{item['name']}'", bold=True, color=self.colors['text']).pack(pady=15)
        vars_map, current = {}, set(item.get("tags", []))
        frame = tk.Frame(dialog, bg=self.colors['card']); frame.pack(padx=20, pady=10, fill='x')
        for tag in self.app_data["tags"]:
            var = tk.BooleanVar(value=(tag in current)); vars_map[tag] = var
            self.mk_checkbutton(frame, tag, var).pack(anchor='w')
        def save_tags():
            item["tags"] = [t for t, v in vars_map.items() if v.get()]
            save_app_data(self.app_data); self.refresh_all_lists(); dialog.destroy()
    ...
    def use_template(self):
        ...
        except Exception: return messagebox.showerror("Ошибка", "Не удалось прочитать выбранный шаблон")
    ...
    def save_template(self):
        ...
        save_app_data(self.app_data); self.refresh_all_lists(); messagebox.showinfo("Успех", "Шаблон сохранен!")
    ...
    def delete_template(self):
        ...
        if messagebox.askyesno("Подтверждение", f"Удалить шаблон '{name}'?"):
            ...
            save_app_data(self.app_data); self.refresh_all_lists(); messagebox.showinfo("Успех", "Шаблон удален!")

    def get_input_from_dialog(self, title, prompt, show=None, timeout=120):
        result, event = [], threading.Event()
        def ask():
            try: result.append(simpledialog.askstring(title, prompt, show=show, parent=self.root))
            finally: event.set()
    ...
    def show_thread_safe_message(self, msg_type, title, message):
        ...
        elif msg_type == "info": messagebox.showinfo(title, message)
    ...
    def save_settings(self):
        ...
        except ValueError: return messagebox.showerror("Ошибка", "Задержка должна быть неотрицательным числом.")
